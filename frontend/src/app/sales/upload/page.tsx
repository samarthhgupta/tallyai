...
import { computeSalesReadiness, createSalesBatch, insertAcceptedSalesInvoices } from '@/lib/salesDb';
import { learnCustomerName } from '@/lib/customers';
import { findDuplicate, recordInvoice } from '@/lib/invoiceHistory';
import type { ExtractedInvoice, ExtractionResponse } from '@/types/invoice';
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl max-w-md w-full mx-4 p-6">
        ...
        <p className="text-sm text-gray-500 dark:text-gray-400 mb-4">
          Rejected invoices will not enter the Sales Register and will be removed from the queue.
        </p>
        <textarea
          value={reason}
...
export default function SalesUploadPage() {
  ...
  const queueStorageKey = company?.id ? `sales_upload_queue_${company.id}` : null;
  ...
  const doAccept = async (keys: string[]) => {
    ...
    // Backend enforcement: block any FY mismatch invoice from entering Sales Register
    ...
    try {
      ...
      if (!activeBatchId) {
        activeBatchId = await createSalesBatch(selectedCompanyId, files.length, financialYear, 'pdf_extraction');
      ...
      await insertAcceptedSalesInvoices(selectedCompanyId, activeBatchId, items, financialYear, companyGstin, companyName);

      // Auto-learn customer names: update customer master with name from invoice (fire-and-forget)
      items.forEach(({ inv }) => {
        if (inv.buyer_gstin && inv.buyer_name) {
          learnCustomerName(selectedCompanyId, inv.buyer_gstin, inv.buyer_name).catch(() => {});
        }
      });
      ...
      const msg = err instanceof Error
        ? err.message
        : (err as { message?: string })?.message ?? 'Failed to save to Sales Register. Please try again.';
      setActionError(msg);
  ...
  // No DB write for sales rejections — simply drop the items from the queue locally.
  const doReject = (keys: string[]) => {
    setQueue((prev) => prev.filter((q) => !keys.includes(q.key)));
    setSelected(new Set());
    setRejectPopup(null);
  };
    ...
    <AppLayout>
      ...
      {rejectPopup && (
        <RejectPopup
          ...
          onConfirm={() => doReject(rejectPopup)}
        ...
        <div className="max-w-5xl">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm p-6 mb-8">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg font-semibold text-gray-900 dark:text-gray-100">Upload Sales Invoices</h2>
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <InvoiceCard
                          ...
                          onSave={(updated) => {
                            ...
                            const r = computeSalesReadiness(updated);
                            const invFY = invoiceFY(updated.invoice_date ?? '');
                            setQueue((prev) =>
                              prev.map((q) =>
                                q.key === item.key
                                  ? { ...q, readiness: r.readiness, readinessFlags: r.flags, fyMismatch: !!invFY && invFY !== financialYear }
                                  : q
                              )
                            );
                          }}
          ...
          {result && queue.length === 0 && (
            <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-10 text-center">
              ...
              <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">Accepted invoices are now in the Sales Register.</p>
              <button
